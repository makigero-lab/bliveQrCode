...
//
// Este ficheiro é partilhado entre o frontend (botão "Importar
// Catálogo" no Admin.jsx) e o script Node `scripts/seed-products.js`
// (que faz o mesmo import para seed via CLI).
//
// Imagens são placeholders da Unsplash (licença livre). Substituir
// por URLs de fotos profissionais quando disponíveis.
// -------------------------------------------------------------
...
// Resumo por categoria (computado uma vez, na carga do módulo)
export const CATALOG_SUMMARY = PRODUCTS.reduce((acc, p) => {
...
// Categorias reconhecidas pela app (deve estar em sincronia com
// src/components/menu/CategoryTabs.jsx e
...
// Função utilitária: devolve um produto "normalizado" com todos os
// campos esperados pelo modelo do Firestore (preenchendo defaults
// para `available`, `stock_enabled`, `stock` se ausentes).
export function normalizeProduct(p) {
  return {
    name: String(p.name ?? "").trim(),
    description: String(p.description ?? "").trim(),
    price: Number.isFinite(Number(p.price)) ? Number(p.price) : 0,
    category: CATEGORIES.includes(p.category) ? p.category : "bebidas",
    image_url: p.image_url || null,
    available: p.available !== false, // default true
    stock_enabled: p.stock_enabled === true, // default false

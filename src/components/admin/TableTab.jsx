...
import { closeTableOrders, updateOrder, cancelOrderItem } from "@/lib/db";
...
export default function TableTab({ tableNumber, orders, onAddOrder }) {
  ...
  // === Anular item individual (Line-item Void) ===
  ...
  const handleCancelItem = async (e, order, itemIndex, item) => {
    ...
    const confirm = window.confirm(
      `Anular este item?\n\n` +
        ...
        `O valor será subtraído do total da mesa.\n` +
        `A anulação fica registada para auditoria.`
    );
    if (!confirm) return;

    const key = `${order.id}-${itemIndex}`;
    setCancelingItems((prev) => new Set([...prev, key]));

    try {
      await cancelOrderItem(order.id, itemIndex, user);
    } catch (err) {
      setError(`Erro ao anular item: ${err?.message || ""}`);
    } finally {
            ...
            sortedOrders.map((order) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Itens — cada item tem botão X para anulação individual */}
                  <div className="px-3 py-2 space-y-1">
                    {(order.items || []).map((item, i) => {
                      ...
                      const cancelKey = `${order.id}-${i}`;
                      const isCanceling = cancelingItems.has(cancelKey);

                      return (
                        <div
                          key={`${item.product_id || i}`}
                          className={`flex justify-between text-xs items-center group ${
                  ...
                  {meta.nextLabel && (
                    <div className="px-3 py-2.5 bg-secondary/30 border-t border-border/30">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        className={`w-full flex items-center justify-center gap-1.5 text-sm font-bold px-4 py-3 rounded-xl min-h-[44px] active:scale-95 transition-all disabled:opacity-50 ${meta.nextBtn}`}
                      >
                        {isAdvancing ? (
